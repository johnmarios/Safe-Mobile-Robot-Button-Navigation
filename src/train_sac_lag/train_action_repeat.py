...
def train_3(MAX_TIMESTEPS,
    ...
    if len(eval_rewards)>0:
        score_history = np.array(eval_rewards) - np.array(eval_turn_penalties)
    ...
    for t in range(MAX_TIMESTEPS):
        ...
        modified_reward = total_reward - turn_penalty
        ...
        if (
            ...
            critic_loss, cost_critic_loss, actor_loss, alpha_loss, alpha = agent.train(BATCH_SIZE)
            ...
            if t % (LOG_FREQ) == 0:
                ...
                cost_critic_loss_history.append(cost_critic_loss)

                print(
                    f"Step {(t+1)*ACTION_REPEAT} | "
                    f"Critic loss: {critic_loss:.4f} | "
                    f"Cost critic loss: {cost_critic_loss:.4f} | "
        ...
        if (
            ...
            score = avg_reward - avg_turn_penalty
...
def save_results(RESULTS_PATH,
    ...
    score_history_eval = np.array(eval_rewards_history) - np.array(eval_turn_penalties_history)
    ...
    np.save(
        f"{RESULTS_PATH}/cost_critic_loss_history.npy",
        np.array(cost_critic_loss_history)
    )
    ...
    plt.figure(figsize=(8,5))
    plt.plot(training_steps, cost_critic_loss_history)
    plt.xlabel("Training step")
    plt.ylabel("Cost critic loss")
    plt.title("Cost critic loss curve")
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"{RESULTS_PATH}/cost_critic_loss_curve.png")
    plt.close()
...
def load_data(LOAD_RESULTS_PATH):
    ...
    if os.path.exists(f"{LOAD_RESULTS_PATH}/eval_rewards.npy"):
        ...
        cost_critic_loss_history = np.load(
            f"{LOAD_RESULTS_PATH}/cost_critic_loss_history.npy"
        ).tolist()
    else:
        eval_rewards = []
        eval_costs = []
